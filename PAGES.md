...
| 구분 | 화면 |
|---|---|
| 등록 플로우 (12) | 상품 정보 입력 · 운송장 준비 · 상품/표시사항 촬영 · 성분 결과 · 성분 직접입력 · HS Code 결과 · 사전심사 안내 · 박스 촬영 · 체적 결과 · 체적 직접입력 · 모서리 보정 · 등록 내용 확인 |
| 공동물류 플로우 (2) | 집하 신청 안내 · 물류비 비교 (혼재 대기 현황은 흐름에서 제외) |
...
│   ProductInfo → LabelCapture → IngredientResult ⇄ IngredientManual
│                              → HsCodeResult → PreReviewGuide (모달)
│                              │
│                              ▼
│                  Waybill → BoxCapture → CbmResult ⇄ CbmManual
│                                       ↳ CornerAdjust
│                                          │
│                                          ▼
│                                       Summary
│
├── 공동물류 플로우
│   Summary → Pickup(집하 신청 안내) → Shipment(물류비 비교) → (Tabs로 리셋)
│   Consolidation(혼재 대기 현황)은 흐름에서 뺐다 — 우리가 집하·출고를 하지 않는다
│
└── 조회 / 부가
...
상단 진행바(`StepBar`)는 등록 플로우 12개 화면 중 8개에 노출됩니다: 상품(0) → 성분(1) → HS Code(2) → 운송장(3) → 체적(4) → 완료(5). 카메라 화면(상품/표시사항 촬영, 박스 촬영)과 모서리 보정, 사전심사 안내에는 없습니다.

**무엇을 보내는지를 먼저 정하고 상자를 나중에 잽니다.** 상자 안에 무엇이 들었는지부터 정해야 말이 되기 때문입니다. 운송장은 HS Code 판정에 쓰이지 않는 **체적 측정용 기준자**이므로, 판정이 끝난 뒤 박스를 찍기 직전에 준비합니다. 아래 절 번호는 문서 작성 순서이고, 실제 진행 순서는 위 다이어그램을 따릅니다.
...
**목적**: 신규 화물 등록의 최소 입력. **상품명은 받지 않습니다** — 사진에서 얻습니다.

상품명을 먼저 입력받으면 사진을 찍는 의미가 없어집니다. 이름만으로도 세번이 나오기 때문입니다. 그래서 임시 이름(`촬영 대기 화물`)으로 등록해두고, HS Code 판정이 품목을 알아내면 서버가 그 이름으로 덮어씁니다([catalog_ai.resolve_product_name](server/app/catalog_ai.py)).

**화면 구성**
- **화물 종류** 카드 2개 — 원물(가공하지 않은 농·수산물) / 가공품(포장에 표시사항이 있는 식품). 기본값 원물
- **출발지** 텍스트 입력 (필수) + 자주 쓰는 출발지 칩 7개(담양·나주·광주·해남·고흥·영암·무안)
- 수출 희망국 칩 5개 (일본·중국·미국·베트남·대만 중 선택, 기본값 일본)
- 종류에 따라 바뀌는 안내 카드 — 원물이면 판정 가능 품목, 가공품이면 판정 방식

**사용자 액션**

| 액션 | 결과 |
|---|---|
| 출발지 공백으로 [다음] | 인라인 에러 "출발지를 입력해주세요" |
| [다음] | `api.createProduct(PLACEHOLDER_NAME, country)` → `setProductKind()`·`setOrigin()` 저장 → `FlowContext.start()` → LabelCapture로 이동 |

**API**: `POST /product/create`

**주고받는 상태**: `FlowContext`를 이 화면에서 **초기화**합니다(`start()`). 여기서 받은 `product_id`가 이후 모든 화면의 기준입니다. 화물 종류와 출발지는 `FlowContext`가 아니라 별도 모듈 [src/state/productKind.ts](app/src/state/productKind.ts)의 모듈 수준 변수에 담깁니다 — 한 번에 한 건만 등록하므로 이것으로 충분하고, `FlowContext`를 건드리지 않기 위한 선택입니다.

**화물 종류가 이후 흐름 전체를 가릅니다.** 이 화면 하단 버튼 문구([다음 : 상품 촬영] / [다음 : 표시사항 촬영]), 촬영 화면의 안내와 가이드 문구, 그리고 서버가 사진을 OCR에 넣을지 품목 분류기에 넣을지가 이 값으로 정해집니다(API.md 3.4절의 `kind`).
...
- "왜 운송장을 붙이나요?" — 한 번 읽으면 되는 내용이라 접어 둔 아코디언(`Collapsible`)
...
| 라우트 | `BoxCapture` |
| 진입 | 운송장 준비 → [다음 : 박스 촬영], 체적 결과 화면 [다시 촬영], 요약 화면의 체적 [수정] |
| StepBar | 없음 (전체화면 카메라) |
...
| [확인] | `confirmCbm` 호출(`input_method: "ai"` 고정) → 성공 시 Summary로 이동 (등록 플로우의 마지막 입력 단계) |
...
**사용자 액션**: 4개 필드가 모두 양수로 채워져야 [저장하고 계속] 활성화. `api.manualCbm()` 호출 → 성공 시 Summary로 이동.
...
## 1.7 상품 / 표시사항 촬영 — `LabelCaptureScreen`

| | |
|---|---|
| 파일 | `app/src/screens/LabelCaptureScreen.tsx` |
| 라우트 | `LabelCapture` |
| 진입 | 상품 정보 입력 → [다음], HS Code 결과의 [사진 다시 촬영], 요약 화면의 HS Code [수정] |
| StepBar | 없음 (전체화면 카메라) |

**목적**: 사진 한 장으로 HS Code를 판정. **화물 종류에 따라 무엇을 찍는지가 달라집니다** — 라우트 이름은 `LabelCapture`지만 원물에서는 상품 자체를 찍습니다.

| 종류 | 찍는 것 | 화면 문구 |
|---|---|---|
| 원물 | 상품 자체 | "상품을 촬영해주세요" / "상품이 네모 안에 가득 차게" |
| 가공품 | 포장 뒷면 표시사항 | "표시사항을 촬영해주세요" / "표시사항 전체가 네모 안에 들어오게" |

문구는 [productKind.ts](app/src/state/productKind.ts)의 `CAPTURE_GUIDE`에 모여 있습니다. 분석 중 화면의 단계 문구도 갈립니다(원물은 "품목 판별 중", 가공품은 "원재료·함량 추출 중").
...
| 서버 응답 | 처리 |
|---|---|
| `success: false` 또는 `action: "manual_input"` | IngredientManual로 `replace`. 인식 실패뿐 아니라 **판정 불가**(`error_reason: "not_classified"`)도 이 경로입니다 |
| `hscode_result` 있음 | `FlowContext.hs`에 저장 → IngredientResult로 `replace` |

**API**: `POST /product/{id}/hscode/predict` — `image`, `kind`(화물 종류), `force_error`(설정의 `hsForceError`)

**주고받는 상태**: 응답의 `product_label`을 `FlowContext.productName`에 반영합니다. 등록 단계에서 상품명을 받지 않아 임시 이름이 들어 있는데, 갱신하지 않으면 이후 결과·요약 화면에 계속 "촬영 대기 화물"로 뜹니다.
...
**화면 구성**
- **제품 머리 카드** — 제품명(26pt 네이비, 가운데)과 식품유형 알약. 이 화면에서 가장 큰 글자
- 보관·표시 정보는 아코디언(`Collapsible`)으로 접어 둠
- **원재료 신뢰 경고** — 함량을 믿을 수 없으면 연빨강 카드(테두리·문구·⚠ 아이콘)로 "포장 뒷면을 보고 수정해주세요"를 띄우고, 원재료 카드 테두리까지 빨갛게 바뀝니다. 카드를 누르면 바로 편집 모드
- 원재료 리스트 — 편집 모드에서 이름·함량 인라인 입력, 삭제(✕), 추가 가능
- 함량 합계 알약(믿을 수 없으면 경고색)

**믿을 수 있는 함량의 기준** (서버 `laver.ratios_trustworthy`와 같습니다)

원재료가 2줄 이상 읽혔고, 함량이 빈 줄이 없으며, 합계가 60~105% 안에 들어야 합니다. 하나라도 어긋나면 위 경고가 뜹니다 — 함량이 세번을 가르기 때문입니다(제16류 주1, 통칙 3나).
...
| 진입 | 촬영 화면의 [직접 입력], 또는 인식 실패·판정 불가 시 자동 전환 |
| StepBar | 1 (성분) |
...
| [다음 : 박스 촬영] | BoxCapture로 이동 |

**원물에서는 차순위 후보가 상태 선택 장치입니다.** 신선 딸기와 냉동 딸기는 같은 사진이지만 세번이 다릅니다(0810.10 / 0811.10). 사진으로는 상태를 알 수 없으므로 서버가 신선을 1순위로 주고 나머지 상태를 `alternatives`에 담아 내려보냅니다. 사용자는 이 목록에서 냉동을 골라 코드를 바꿉니다 — 기존 후보 선택 UI를 그대로 씁니다.
...
| 라우트 | `Summary` |
| 진입 | 체적 확정([확인]) 또는 체적 직접 입력([저장하고 계속]) |
| StepBar | 5 (완료) |
...
**사용자 액션**: [물류비 비교 보기] → Shipment. **이 화면은 등록 흐름에서 빠졌다** — 컨테이너 적재율과 출고 시점은 우리가 정하는 것이 아니다.

---

## 2.3 물류비 비교 — `ShipmentScreen`
...
**목적**: 등록 플로우 전체의 종착점. 공동물류 접수를 확정하고 개별 발송 대비 절감액을 제시한 뒤 홈으로 복귀.

**출고 자체는 다루지 않습니다.** 농가가 직접 배에 싣는 것이 아니라 권역 공동물류에 화물을 넘기는 구조이므로, 컨테이너 종류·선적항·선적일 같은 출고 정보는 화면에서 뺐습니다(서버 응답에는 남아 있습니다).

**화면 구성**
- 완료 히어로 — 체크 아이콘, "공동물류 접수 완료", 상품명
...
**화면 구성**
- 네이비 히어로 — "쉽다 / 사진 한 장으로 수출 신고 준비"를 **가운데 정렬**(상태바 이 화면에서만 밝은 글자로 전환)
- [화물 등록 시작] 카드 — 히어로에 걸치는 네이비 단색, 아이콘·제목·설명을 세로로 쌓아 가운데 정렬. 화면에서 가장 진한 덩어리
- 진행중 / 완료 건수 타일 2개 (보색 — 주황 `solidProgress` / 초록 `solidDone`)
- **최근 등록** — 기본은 접혀 있는 드롭박스. 제목 줄에 건수와 ∨ 아이콘만 보이고, 누르면 표 형태로 펼쳐집니다
- 펼친 목록의 각 줄은 이름·날짜·상태 셋뿐이고, HS Code·체적은 줄을 눌러 다시 펼칩니다
- 목록 맨 아래 "전체 N건 보기" → 이력 탭

**동작**: 화면에 포커스될 때마다(`useFocusEffect`) 목록을 다시 불러옵니다 — 다른 화면에서 등록을 마치고 돌아오면 자동 갱신.

**API**: `GET /product/recent?limit=5`

**사용자 액션**: [화물 등록 시작] → `FlowContext.reset()` 후 ProductInfo로 이동. 줄 탭 → 상세 펼침, [상세 보기] → Detail로 이동.
